...
| `redeem_price_threshold` | `1.0`                     | Data API 的 `curPrice` ≥ 此值（100%）才触发自动/手动结算 |
...
- **自动**：每 `redeem_poll_sec` 拉取持仓；链上仍有份额、`redeemable=true` 且 `curPrice = 100%` 的胜方，链上调用 `redeemPositions` 换回 USDC。
- **手动**：Dashboard「持仓结算」面板可单场结算，或「结算全部胜方 / 全部可结算」——**同样要求价格 = 100%**。
...
| `history_page_size`     | `10`             | 成交/错过列表每页条数            |
| `watchlist_grace_hours` | `2`              | 开赛后仍保留在 Watchlist 的小时数 |
...
展示 **所有 `watching` 监听中** 的场次；已 `done` 的场次在开赛后 2 小时内仍保留。时间均为 **北京时间**。

**比分/进度**：按队名 + 开球时间与 ESPN 等源对齐；未来盘不会套用同名球队的历史终局比分。
...
| ARMED / 已成交 | ARMED = 已进入直播买入监控；已成交 = 本 bot 已有下单记录 |
...
展示 `FUNDER` 钱包**链上仍有份额**的真实持仓（非历史结算记录）：


| 列    | 含义                                       |
| ---- | ---------------------------------------- |
| 盘口标题 | Polymarket `question`（非 ESPN 对阵简称）       |
| 份额   | 链上 CTF 余额                                |
| 价格%  | 当前价 × 100；**= 100%** 且 `redeemable` 时可结算 |
| 状态   | 可结算 / 胜方 / 已结算                           |
| 操作   | 单场「结算」按钮（仅 `can_settle` 时可点）             |


顶部按钮：**结算全部胜方**、**结算全部可结算**（均要求价格 = 100%）。

结算成功会写入历史记录，类型为 **结算**（`kind: redeem`），日志事件 `REDEEM_OK` / `REDEEM_FAIL`。

### 4.5 历史（成交 / 错过 / 结算）

合并 `trades` 与 `signal_events` 表，按时间倒序，**每页 10 条**。高频噪音（如 cooldown、重复未成交重试、盘口拉取失败）不会出现在列表中。类型标签：
...
| `unknown_winner_token`           | 赛果胜方队名无法映射到 PM token（队名不一致等） | 检查队名 alias |
| `draw_market_unmapped`           | 平局但盘口类型不支持（非 Will-X-win / 平局盘）   | 检查 question 格式 |
